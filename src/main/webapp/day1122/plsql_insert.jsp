...
<div>
	<h2>plsql_insert의 사용</h2>
	<form action="index.jsp" method="get">
		<input type="hidden" name="type" value="${param.type}" /> <label>사원번호</label>
		<input type="text" name="empno" /><br> <label>사원명</label> <input
			type="text" name="ename" /><br> <label>직무</label> <input
			type="text" name="job" /><br>
		<label>연봉</label> <input type="text" name="sal" /><br> <label>보너스</label>
		<input type="text" name="comm" /><br> <label>부서번호</label> <input
			type="text" name="deptno" /><br> <input type="submit"
			value="사원 추가" class="btn btn-primary btn-sm" /><br>
	</form>
		...
		<c:if test="${not empty param.empno}">
			...
			<c:choose>
				<c:when test="${ce2VO.cnt ne 0 }">
					<c:out value="${ce2VO.ename }" />사원의 정보가 추가되었습니다. <br>
				</c:when>
				<c:otherwise>
					사원정보가 입력되지 않았습니다.
